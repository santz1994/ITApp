...
class AssetService
{
    ...
    {
        // Try to find by qr_code field first, then by asset_tag as fallback
        $asset = Asset::where('qr_code', $qrCode)
                   ->with(['assetModel.manufacturer', 'assetModel.asset_type', 'assetModel.pcspec', 
                          'status', 'assignedUser', 'location', 'division', 'supplier', 'tickets'])
                   ->first();
        
        // If not found by qr_code, try asset_tag
        if (!$asset) {
            $asset = Asset::where('asset_tag', $qrCode)
                       ->with(['assetModel.manufacturer', 'assetModel.asset_type', 'assetModel.pcspec', 
                              'status', 'assignedUser', 'location', 'division', 'supplier', 'tickets'])
                       ->first();
        }
    ...
    public function getAssetStatistics()
    {
        return Cache::remember('asset_statistics', 300, function () {
            return [
                'total' => Asset::count(),
                'deployed' => Asset::whereIn('status_id', [2, 7])->count(),  // Status ID 2 = Deployed, 7 = Active
                'active' => Asset::where('status_id', 7)->count(),   // Status ID 7 = Active
                'in_use' => Asset::whereIn('status_id', [2, 7])->count(),  // Deployed or Active
                'in_stock' => Asset::where('status_id', 1)->count(),  // Status ID 1 = Ready to Deploy
                'in_repair' => Asset::whereIn('status_id', [3, 4, 8])->count(),  // Status IDs 3, 4, 8 = repairs
                'disposed' => Asset::whereIn('status_id', [5, 6])->count(),  // Status IDs 5, 6 = written off
